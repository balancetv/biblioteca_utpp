...
// Obtener direcciones
$direcciones = $conexion->query("SELECT direccionId, nombre FROM direcciones");

// Obtener dependencias agrupadas por dirección
$dependenciasData = $conexion->query("SELECT dependenciaId, nombreDependencia, direccionId FROM dependencia");
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $sql = "INSERT INTO usuarios (direccionId, dependenciaId, usuario, nombre, contraseña, nivel, estatus, fechaRegistro)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
    $stmt = $conexion->prepare($sql);
    $stmt->bind_param("iissssss", $direccionId, $dependenciaId, $usuario, $nombre, $contrasena, $nivel, $estatus, $fecha);
    $stmt->execute();
}

// Consulta para mostrar usuarios
$consulta = "SELECT u.usuario, u.nombre, u.nivel, u.estatus, u.fechaRegistro,
                    d.nombre AS direccion, dp.nombreDependencia AS departamento
             FROM usuarios u
             JOIN direcciones d ON u.direccionId = d.direccionId
             JOIN dependencia dp ON u.dependenciaId = dp.dependenciaId";
    ...
    <body>
        ...
        <div class="container mt-5">
            ...
            <form class="row g-3" method="POST">
                <div class="col-md-4">
                    ...
                    <select name="direccion" id="direccion" class="form-select" onchange="cargarDependencias()" required>
                        ...
                        <?php while ($dir = $direcciones->fetch_assoc()): ?>
                            <option value="<?= $dir['direccionId'] ?>"><?= htmlspecialchars($dir['nombre']) ?></option>
                        <?php endwhile; ?>
                    </select>
                </div>
                ...
                <div class="col-md-4"><label>Nombre del Usuario</label><input type="text" name="nombre" class="form-control" required></div>
            ...
            function cargarDependencias() {
                ...
                if (dependencias[direccionId]) {
                    dependencias[direccionId].forEach(dep => {
                        ...
                        option.textContent = dep.nombreDependencia;
