...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['direccionId'], $_POST['id'], $_POST['nombre']) && !isset($_POST['buscar'])) {
    ...
    $id = intval($_POST['id']);
    $nombreDependencia = $conexion->real_escape_string($_POST['nombre']);

    $conexion->query("INSERT INTO dependencia (dependenciaId, nombreDependencia, direccionId) VALUES ('$id', '$nombreDependencia', '$direccionId')");
}

// CONSULTAR DIRECCIONES PARA SELECT
$direcciones = [];
$consultaDirs = $conexion->query("SELECT direccionId, nombre FROM direcciones");
...
if (isset($_POST['buscar'])) {
    ...
    $filtro = "WHERE d.nombreDependencia LIKE '%$busqueda%' OR d.dependenciaId LIKE '%$busqueda%' OR dr.nombre LIKE '%$busqueda%'";
}

$sql = "SELECT d.dependenciaId, d.nombreDependencia, dr.nombre AS nombreDireccion 
        FROM dependencia d 
        ...
        <?php if (count($dependencias) > 0): ?>
            <?php foreach ($dependencias as $dep): ?>
                <tr>
                    ...
                    <td><?php echo htmlspecialchars($dep['nombreDependencia']); ?></td>
                </tr>
            <?php endforeach; ?>
        <?php else: ?>
